...
- **Tax law does not leak.** The neutral `tax/` layer holds the general tax *concepts* — the engine interface, the taxpayer context (`TaxContext`/`TaxSubject`/`TaxProperty`/`PropertyDisposition`), and `FilingStatus`; forecast/period import only `tax/`. US tax *law* — brackets, parameters, the surviving-spouse filing rule, recovery periods — lives in `tax/us` and is reached only through the engine: the Forecast states facts (standing filing status, a spouse death year) and the engine derives the US specifics (the effective filing status). The Forecast maps its own vocabulary to neutral concepts (`ContributionSource`→`ContributionKind`); `TaxLaw` is the only importer of `tax/us`. → `tax/context.py`, `tax/engine.py`, `tax/law.py`
