...
from ucfp.forecast.tests.granularity_profiles import PROFILES, TIERS
...
class GranularityInvarianceTest( unittest.TestCase ):
    ...
    def test_pure_stream_flows_invariant_across_granularity( self ):
        ...
        pre-depletion year, for every profile and tier -- the rate-flow invariant."""
        for profile_name, build in PROFILES.items():
            for tier_name, transform in TIERS.items():
                comparison = compare( transform( build() ) )
                    ...
                    for year, annual_figures in annual.items():
                        ...
                        for income_class in _PURE_STREAM_INCOME:
                            self._assert_close(
                                ...
                                profile_name, tier_name, granularity_name, year, income_class.name )
                        for expense_class in _PURE_STREAM_EXPENSE:
                            self._assert_close(
                                ...
                                profile_name, tier_name, granularity_name, year, expense_class.name )
                        continue
                    continue
                continue
            continue
    ...
    def test_occurrence_year_totals_invariant_across_granularity( self ):
        ...
        comparison = compare( TIERS[ 'null' ]( PROFILES[ 'wage_earner' ]() ) )
        annual     = { figures.year : figures for figures in comparison[ 'annual' ][ 0 ] }
        for granularity_name, _ in GRANULARITIES:
            other = { figures.year : figures for figures in comparison[ granularity_name ][ 0 ] }
            for year, annual_figures in annual.items():
                self._assert_close(
                    ...
                    'wage_earner', 'null', granularity_name, year, 'SALT' )
                continue
            continue
    ...
    def test_null_tier_loan_free_net_worth_invariant( self ):
        ...
        prorating shows here as a within-year net-worth divergence."""
        for profile_name, build in PROFILES.items():
            parameters = TIERS[ 'null' ]( build() )
            ...
            for granularity_name, _ in GRANULARITIES:
                ...
                for year, annual_figures in annual.items():
                    ...
                    self._assert_close(
                        ...
                        profile_name, 'null', granularity_name, year, 'net_worth' )
                    continue
                continue
            continue
    ...
    def test_depletion_year_identical_at_low_tiers( self ):
        ...
        across every granularity -- the strongest form of the materiality bar."""
        for profile_name, build in PROFILES.items():
            for tier_name in _LOW_TIERS:
                comparison = compare( TIERS[ tier_name ]( build() ) )
                years = { name : outcome.depletion_year
                          for name, ( _figures, outcome ) in comparison.items() }
                with self.subTest( profile = profile_name, tier = tier_name ):
                    self.assertEqual(
                        len( set( years.values() ) ), 1,
                        f'{profile_name}/{tier_name}: depletion year differs across granularity: {years}' )
                continue
            continue
    ...
    def test_depletion_year_within_one_at_funding_tiers( self ):
        ...
        refining the granularity converges (quarterly agrees with monthly)."""
        for profile_name, build in PROFILES.items():
            for tier_name in _FUNDING_TIERS:
                ...
                with self.subTest( profile = profile_name, tier = tier_name ):
                    self.assertLessEqual(
                        ...
                        f'{profile_name}/{tier_name}: depletion year spans more than one year: '
                        f'{[ ( n, o.depletion_year ) for n, ( _f, o ) in comparison.items() ]}' )
                continue
            continue
    ...
    def test_finer_granularity_converges_on_terminal_net_worth( self ):
        ...
        post-depletion carry to muddy the terminal figure."""
        for profile_name, build in PROFILES.items():
            for tier_name, transform in TIERS.items():
                comparison = compare( transform( build() ) )
                ...
                with self.subTest( profile = profile_name, tier = tier_name ):
                    self.assertTrue(
                        ...
                        f'{profile_name}/{tier_name}: quarterly terminal net worth {quarterly} not '
                        f'between annual {annual} and monthly {monthly}' )
                continue
            continue
