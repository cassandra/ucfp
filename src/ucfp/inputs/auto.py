"""§ Spending: the household's car-ownership costs.
...
and -- if financed -- either a down payment or a monthly payment. It is stored as an `AutoPlan`
...
from ucfp.inputs.plans.schemas import AutoPlan
...
class AutoPlanForm( forms.Form ):
    ...
    auto plan, leaving the rest of Plans intact."""
    ...
    def _initial( cls, profile, plans ) -> dict:
        plan = plans.auto_plan if plans is not None else None
        if plan is not None:
            return {
    ...
    def _auto_loan_end( profile, plans ):
        ...
        if plans is None:
            return None
        auto = next( ( debt for debt in profile.debts if debt.kind is DebtKind.AUTO ), None )
        if auto is None:
            return None
        repayment = next(
            ( r for r in plans.loan_repayments if r.debt_handle == auto.handle ), None )
    ...
    def apply( self, profile, plans ):
        return profile, replace( plans, auto_plan = self._plan() )
