...
composition point that maps a jurisdiction (`TaxLawType`) to its concrete claiming-age schedule
(`tax.us` for US Social Security today), so a caller asks for a realized benefit without
encoding which jurisdiction's rules apply -- the same shape as `TaxLaw` for the tax engine.

It lives in `tax` because it is jurisdiction-statutory, the same family as the tax law. The
jurisdiction is keyed on `TaxLawType` for now (the existing jurisdiction discriminator); when
`tax/` generalizes beyond tax, that becomes a first-class jurisdiction concept.
"""
...
class GovernmentPension:
    ...
    `TaxLawType` to its concrete claiming-age schedule and tax treatment, keeping callers
    jurisdiction-agnostic."""

    def __init__( self, jurisdiction : TaxLawType ):
