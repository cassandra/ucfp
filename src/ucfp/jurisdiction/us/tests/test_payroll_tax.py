"""Employee payroll tax (`USFederalTaxEngine._payroll_tax`): Social Security on each worker's wages up
to the wage base (capped per worker), Medicare on all wages, plus the Additional Medicare surtax on
...
class _Window:
    """A fiscal-window stub exposing per-worker WAGES (the only thing `_payroll_tax` reads); every
...
class PayrollTaxTests( unittest.TestCase ):

    def setUp( self ):
        self.engine = USFederalTaxEngine( federal_2026() )

    def _payroll( self, status, *worker_wages ):
        return self.engine._payroll_tax( status, _Window( *worker_wages ) )

    def test_below_wage_base_and_surtax_threshold( self ):
        # single, 100k wages: SS 6.2% = 6,200; Medicare 1.45% = 1,450; no surtax -> 7,650
        self.assertEqual( self._payroll( FilingStatus.SINGLE, '100000' ), _D( '7650' ) )
    ...
    def test_ss_capped_at_the_wage_base_plus_the_additional_medicare_surtax( self ):
        ...
        self.assertEqual( self._payroll( FilingStatus.SINGLE, '300000' ), _D( '16689' ) )
    ...
    def test_social_security_cap_is_per_worker( self ):
        ...
        self.assertEqual(
            self._payroll( FilingStatus.MARRIED_JOINT, '184500', '184500' ), _D( '29299.50' ) )
