...
encrypted `data` fields re-encrypt under the *local* key on save. The example org and scenario carry the
...
edited example. An unchanged fixture is a no-op, so the deploy entrypoint can run this on every start. We
...
from ucfp.onboarding.constants import (
    EXAMPLE_ASSUMPTIONS_NAME, EXAMPLE_FIXTURE_PATH, EXAMPLE_FORECAST_NAME, EXAMPLE_ORGANIZATION_NAME,
    EXAMPLE_ORGANIZATION_UUID, EXAMPLE_PLANS_NAME, EXAMPLE_SCENARIO_NAME, EXAMPLE_SCENARIO_UUID )


class NoSuperuserError( Exception ):
    """No superuser exists to own the example household -- `bootstrap` must run before this seed."""


@dataclass( frozen = True )
class ExampleOrgResult:
    organization : Organization
    action       : str                                   # 'created' | 'refreshed' | 'preserved'


def seed_example_org( force : bool = False ) -> ExampleOrgResult:
    """Ensure the example household exists (owned by the superuser) and holds a runnable scenario with a
    captured forecast. Content-aware idempotency: an already-seeded example is refreshed only when the
    ...
    with transaction.atomic():
        organization, _ = Organization.objects.get_or_create(
            uuid = EXAMPLE_ORGANIZATION_UUID, defaults = { 'name': EXAMPLE_ORGANIZATION_NAME } )
        OrganizationMember.objects.get_or_create(
            organization = organization, user = superuser,
            defaults = { 'organization_role': OrganizationRole.OWNER } )
        already_seeded = ScenarioRecord.objects.filter( uuid = EXAMPLE_SCENARIO_UUID ).exists()
        if already_seeded and not force and _is_current( organization ):
            return ExampleOrgResult( organization = organization, action = 'preserved' )
        if already_seeded:                                   # drops the old data + its 'Example Forecast' run
...
def _superuser():
    ...
    if superuser is None:
        raise NoSuperuserError( 'No superuser exists to own the example household; run `bootstrap` first.' )
    return superuser


def _load_fixture():
    return json.loads( EXAMPLE_FIXTURE_PATH.read_text() )
...
def _fixture_matches( organization ) -> bool:
    ...
    or review-state."""
    scenario = ScenarioRecord.objects.filter( uuid = EXAMPLE_SCENARIO_UUID ).first()
...
def _is_current( organization ) -> bool:
    """Whether the seeded example is already up to date: its records equal the committed fixture *and* its
...
def _clear_example_data( organization ):
    """Drop the example org's data records (keeping the org and its memberships), most-referencing first,
...
def _seed_records( organization ):
    ...
    plans_record = PlansRecord.objects.create(
        organization = organization, label = EXAMPLE_PLANS_NAME, data = fixture[ 'plans' ][ 'data' ],
    ...
    assumptions_record = AssumptionsRecord.objects.create(
        organization = organization, label = EXAMPLE_ASSUMPTIONS_NAME,
    ...
    scenario = ScenarioRecord.objects.create(
        uuid = EXAMPLE_SCENARIO_UUID, organization = organization, label = EXAMPLE_SCENARIO_NAME,
...
def _generate_forecast( organization, profile_record, scenario ):
    ...
    horizon is the shared age-based default, so the example spans the household's life."""
    ...
    with transaction.atomic():
        run = run_and_capture(
            ...
            label = EXAMPLE_FORECAST_NAME, source_label = scenario.label )
