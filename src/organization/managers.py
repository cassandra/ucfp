...
# The reserved name of the single shared, memberless organization used when `SUPPRESS_AUTHENTICATION`
# is set (typically a self-hosted single-user run). The name doubles as the lookup key for the singleton.
...
class OrganizationManager( models.Manager ):
    ...
    def get_or_create_shared( self ) -> 'Organization':
        """The single shared, memberless organization used when `SUPPRESS_AUTHENTICATION` is set
        (typically a self-hosted single-user run). There is no user to own it, so it carries no
