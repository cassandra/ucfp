...
from .constants import DEFAULT_TIMEZONE_NAME, TIMEZONE_CHOICES
...
from .managers import (
    ...
    is_autogenerated_organization_name,
)
...
class Organization( TimestampedModel ):
    ...
    def display_name( self ) -> str:
        """The name to show a person. Auto-provisioned personal households are
        named `user-<uuid>`, which is meaningless to a reader; those show a
        neutral placeholder instead of the raw name."""
        if is_autogenerated_organization_name( self.name ):
            return _PLACEHOLDER_ORGANIZATION_NAME
        return self.name

    @property
    def tzinfo( self ) -> ZoneInfo:
        """The household's display timezone as a `ZoneInfo`, for rendering stored (UTC) datetimes --
        run timestamps and the default run name -- in the household's local time. `ensure_organization`
        activates this for the request so templates and `timezone.localtime()` read it automatically."""
