...
print_important "UCFP Development Setup
...
if [[ ! -f "VERSION" || ! -d "deploy" || ! -d "docs" ]]; then
    ...
    print_notice "Please cd to the ucfp directory and try again."
    exit 1
fi
...
if [[ -z "$CURRENT_ORIGIN" ]]; then
    ...
    if prompt_yes_no "Do you have SSH keys set up with GitHub?" "n"; then
        git remote add origin "git@github.com:${GITHUB_USERNAME}/ucfp.git"
    else
        git remote add origin "https://github.com/${GITHUB_USERNAME}/ucfp.git"
    fi
...
pip install --quiet -r src/ucfp/requirements/development.txt
