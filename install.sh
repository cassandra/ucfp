...
# UCFP - Single Command Installer
# Installs and runs UCFP with minimal user interaction
# Requires: Docker

# Configuration
DOCKER_IMAGE="ghcr.io/cassandra/ucfp"  # GitHub Container Registry
DOCKER_TAG="${1:-latest}"  # Allow override for testing (default: latest)
CONTAINER_NAME="ucfp"
EXTERNAL_PORT="9666"
# Install directory. Defaults to the hidden ~/.ucfp. choose_install_dir() may
# switch this to the non-hidden UCFP_HOME_VISIBLE at install time when Docker
...
# from the final UCFP_HOME by set_install_paths().
UCFP_HOME="${HOME}/.ucfp"
UCFP_HOME_VISIBLE="${HOME}/ucfp"
HAS_COMPOSE=0  # Set by check_docker_compose; 1 if `docker compose` is available.

# Derive all install paths from the (possibly updated) UCFP_HOME.
set_install_paths() {
    ENV_DIR="${UCFP_HOME}/env"
    DATABASE_DIR="${UCFP_HOME}/database"
    MEDIA_DIR="${UCFP_HOME}/media"
    ENV_FILE="${ENV_DIR}/local.env"
    COMPOSE_FILE="${UCFP_HOME}/docker-compose.yml"
}
...
# compose file or --env-file under ~/.ucfp and fails with an opaque "permission
...
create_directories() {
    ...
    print_success "Directories created: ${UCFP_HOME}"
}
...
# UCFP Environment Configuration
...
# Write a fully-resolved docker-compose.yml to ${UCFP_HOME}/. Always written, even
# when compose is not available, so it is ready for use the moment the user
# installs the compose plugin. Container name `ucfp` matches the legacy
# `docker run --name ucfp` invocation so management commands like `docker logs
# ucfp`, `docker stop ucfp`, and `docker start ucfp` work identically across both
...
# Pull Docker image
pull_docker_image() {
    print_info "Pulling UCFP Docker image..."
...
# paths produce a container named `ucfp`, so post-install management commands
# documented in show_success() work identically.
start_container() {
    print_info "Starting UCFP container..."
...
show_success() {
    ...
    echo -e "${GREEN}================================${NC}"
    echo
    echo -e "${BLUE}🌐 Access your UCFP system:${NC}"
    ...
    echo -e "   Update: curl -fsSL https://raw.githubusercontent.com/cassandra/ucfp/master/update.sh | bash"
    echo
