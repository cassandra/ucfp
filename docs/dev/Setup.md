...
```
git clone https://github.com/${YOURUSERNAME}/ucfp.git
cd ucfp
make env-build-dev
python3.11 -m venv venv
. ./dev/init-env-dev.sh
pip install -r src/ucfp/requirements/development.txt
cd src
./manage.py check
./manage.py migrate
./manage.py bootstrap
...
- Go to the main repository on GitHub: https://github.com/cassandra/ucfp
...
- The forked repo will be located at https://github.com/${YOURUSERNAME}/ucfp.git (if you kept the same repo name).
...
git clone git@github.com:${YOURUSERNAME}/ucfp.git
```
...
``` shell
git remote add origin git@github.com:${YOURUSERNAME}/ucfp.git

# If no SSH keys were added to GitHub, you'll need this instead:
git remote set-url origin https://github.com/${YOURUSERNAME}/ucfp.git
```
...
./manage.py test ucfp.my_module      # Run specific app tests
