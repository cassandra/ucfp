...
cd $PROJ_DIR
cd src
pip install -r ucfp/requirements/development.txt
./manage.py migrate
```
...
- Go to your fork on GitHub: https://github.com/${YOURUSERNAME}/ucfp
- Click the "Compare & pull request" button that appears after your push.
- Ensure the base repository is set to `cassandra/ucfp -> staging`
- Ensure the compare branch is set to: `${YOURUSERNAME}/ucfp -> feature/${YOUR_FEATURE_NAME}`
