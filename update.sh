...
# UCFP - Update Script
# Updates an existing UCFP installation to the latest version
# Requires: Docker and existing installation

# Configuration
DOCKER_IMAGE="ghcr.io/cassandra/ucfp"
DOCKER_TAG="${1:-latest}"  # Allow override for testing (default: latest)
CONTAINER_NAME="ucfp"
EXTERNAL_PORT="9666"
# Resolve the existing install directory. install.sh uses the hidden ~/.ucfp by
# default, but falls back to a non-hidden ~/ucfp when Docker cannot read
...
if [[ -f "${HOME}/ucfp/env/local.env" ]]; then
    UCFP_HOME="${HOME}/ucfp"
else
    UCFP_HOME="${HOME}/.ucfp"
fi
ENV_FILE="${UCFP_HOME}/env/local.env"
...
# install.sh-generated compose file exists at ~/.ucfp/docker-compose.yml.
...
show_success() {
    ...
    echo -e "${GREEN}================================${NC}"
    echo
    echo -e "${BLUE}🌐 Access your updated UCFP system:${NC}"
